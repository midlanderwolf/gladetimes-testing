...
class Command(BaseCommand):
    ...
    def handle_operator(self, line):
        ...
        if not operator:
            operator = Operator(name=name, noc=agency_id, url=getattr(line, 'agency_url', ''))
            operator.save()
        elif operator.url != getattr(line, 'agency_url', ''):
            operator.url = getattr(line, 'agency_url', '')
    ...
    def handle_zipfile(self, path):
        try:
            ...
            logger.error(f"Failed to read GTFS feed: {e}")
            return
        if feed is None:
            logger.error("gtfs_kit.read_feed returned None - invalid GTFS feed")
            return
    ...
    def handle(self, *args, **options):
        source = DataSource.objects.get(name="Bus Open Data Service")
        if not source.url:
            logger.error("No URL set for Bus Open Data Service")
            return

        path: Path = settings.DATA_DIR / "bods_gtfs.zip"

        modified, last_modified = download_if_modified(path, source)
        if modified or last_modified != source.datetime or options["force"]:
            if not path.exists():
                logger.error(f"Download failed for {source.name}: {source.url}")
                return
